...
bool pipe(int x, int y, vector<vector<int>>&v,int n,bool right,bool up,bool down)
{
	if (x==1&&y==n)
	{
		return true;
	...
	if (v[x][y]==1||v[x][y]==2)
	{
		if (right)
		{
			return pipe(x, y+1, v, n,1,0,0);
		}
		if (up)
		{
			return pipe(x-1, y, v, n, 0, 1, 0);
		}
		if (down)
		{
			return pipe(x+1, y, v, n, 0, 0, 1);
		}
	}
	else
	{
